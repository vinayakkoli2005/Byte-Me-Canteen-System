...
public class Customer {
    ...
    /** Load-only constructor: does NOT auto-register into Database lists (caller handles placement). */
    static Customer loadFromFile(String name, Integer Roll_number, boolean isVIP) {
        Customer c = new Customer(name, Roll_number, isVIP);
        // The regular constructor already added to DB lists — that is correct for load too.
        return c;
    }
    ...
    public void addToCart(String name, Integer Quantity, String Specialreq) {
        ...
        for (Food i : Menu.items) {
            if (i.getName().equalsIgnoreCase(name)) {
                ...
                // Remove existing cart entry for this item (replace with updated quantity)
                Cart.removeIf(f -> f.getName().equalsIgnoreCase(name));
                // Add a cart-specific copy so the shared Menu item is never mutated
                Cart.add(i.cartCopy(Quantity, Specialreq, this.name));
                found = true;
