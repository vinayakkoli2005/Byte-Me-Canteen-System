...
public class Main extends JFrame  {
    static void addItem() {
        addMenuFood("Pizza", 10.99, true, "Snacks");
        addMenuFood("Burger", 8.99, true, "Snacks");
        addMenuFood("Small Meal", 12.99, true, "Meals");
        addMenuFood("Coke", 2.99, true, "Drinks");
        addMenuFood("Coffee", 3.99, true, "Drinks");
        addMenuFood("Regular Meal", 5.99, true, "Meals");
        addMenuFood("Large Meal", 4.99, true, "Meals");
        addMenuFood("Jumbo Meal", 3.99, true, "Meals");
        addMenuFood("Punjabi Meal", 6.99, true, "Meals");
    }

    private static void addMenuFood(String name, double price, boolean available, String category) {
        Food item = new Food(name, price, available, category);
        Menu.items.add(item);
        Menu.Prices.add(price);
        if (category.equalsIgnoreCase("Snacks")) Menu.Snacks.add(item);
        else if (category.equalsIgnoreCase("Drinks")) Menu.Drinks.add(item);
        else if (category.equalsIgnoreCase("Meals")) Menu.Meals.add(item);
    }
    /** Seed default customers only when no saved data file exists. */
    static void addCustomer() {
        if (!new java.io.File("Customer_Data.txt").exists()) {
            new Customer("John", 12345, false);
            new Customer("Jane", 12346, true);
            new Customer("Jack", 12347, true);
            new Customer("Jill", 12348, false);
            new Customer("Joe", 12349, false);
            new Customer("Jenny", 12350, false);
        }
    }
                    ...
                    if (customerOption == 1) {
                        ...
                        while( !exit ) {
                            ...
                            if (browseOption == 1) {
                                ...
                                // Copy into a temp queue so the original is not destroyed
                                PriorityQueue<Double> tempPrices = new PriorityQueue<>(Menu.Prices);
                                while (!tempPrices.isEmpty()) {
                                    Double price = tempPrices.poll();
                ...
                while (true) {
                    ...
                    if (adminOption == 1) {
                        ...
                        if (menuOption == 1) {
                            ...
                            sc.nextLine();
                            System.out.println("Enter the category (Drinks/Snacks/Meals):");
                            String category = sc.nextLine();
                            if (category.equalsIgnoreCase("Drinks") || category.equalsIgnoreCase("Snacks") || category.equalsIgnoreCase("Meals")) {
                                addMenuFood(name, price, availability, category);
                                System.out.println("Item added successfully");
                            } else {
                                System.out.println("Unknown category. Item not added.");
                            }
